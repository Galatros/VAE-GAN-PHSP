...
class Generator(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.map1_en = nn.Sequential(
            torch.nn.Linear(self.latent_dim, 400),#latent + coditional
            nn.LeakyReLU(),
            nn.BatchNorm1d(400),
        )
        self.map1_au = nn.Sequential(
            torch.nn.Linear(6, 400),#latent (orginal photons) withou conditional
            nn.LeakyReLU(),
            nn.BatchNorm1d(400),
        )
        ...
        #     torch.nn.Linear(self.latent_dim, 400),#Używano odwrotnej kolejności funkcji aktywacji i norm( w dodatku layernorm)
        #     nn.LeakyReLU(),
        #     nn.BatchNorm1d(400),
        #     torch.nn.Linear(400, 400),
        #     nn.BatchNorm1d(400),
